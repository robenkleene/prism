...
pub fn absolute_path(relative_path: &str, config: &Config) -> Option<PathBuf> {
    match (
        &config.cwd_of_prism_process,
        &config.cwd_of_user_shell_process,
        calling_process().paths_in_input_are_relative_to_cwd() || config.relative_paths,
    ) {
        // Note that if we were invoked by git then cwd_of_prism_process == repo_root
        (Some(cwd_of_prism_process), _, false) => Some(cwd_of_prism_process.join(relative_path)),
        (_, Some(cwd_of_user_shell_process), true) => {
            Some(cwd_of_user_shell_process.join(relative_path))
        }
        (Some(cwd_of_prism_process), None, true) => {
            // This might occur when piping from git to prism?
            Some(cwd_of_prism_process.join(relative_path))
        }
        _ => None,
    }
    .map(normalize_path)
...
/// Relativize path if prism config demands that and paths are not already relativized by git.
...
/// in when prism exits. This is the directory relative to which the file paths in prism output are
/// constructed if they are using either (a) prism's relative-paths option or (b) git's --relative
/// flag.
pub fn cwd_of_user_shell_process(
    cwd_of_prism_process: Option<&PathBuf>,
    cwd_relative_to_repo_root: Option<&str>,
) -> Option<PathBuf> {
    match (cwd_of_prism_process, cwd_relative_to_repo_root) {
        (Some(cwd), None) => {
...
#[cfg(test)]
pub fn fake_prism_cwd_for_tests() -> PathBuf {
    #[cfg(not(target_os = "windows"))]
    {
        PathBuf::from("/fake/prism/cwd")
    }
    #[cfg(target_os = "windows")]
    {
        PathBuf::from(r"C:\fake\prism\cwd")
    }
}
